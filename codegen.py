...
def _call_claude_raw(client, prompt, max_tokens=8192):
...
def generate_frame_sync_code(client, in_samples):
    """Returns (code, error). code is None whenever nothing usable came
    back — no fake placeholder is ever returned as if it were real code,
    since assemble_full_driver decides whether to wire poll() up to
    read_frame() based on whether this is truthy; returning a comment
    string on failure would make poll() call a function that was never
    actually defined. Uses a larger token budget than the per-label
    encode/decode generation — inferring a frame synchronizer, especially
    across several real samples, needs more room to think it through."""
    code, stop_reason = _call_claude_raw(client, build_frame_sync_prompt(in_samples), max_tokens=16000)
    if not code.strip():
        return None, f"Claude returned no code (stop_reason: {stop_reason})"

    error = verify_frame_sync(code, in_samples)
    if not error:
        return code, None

    code, stop_reason = _call_claude_raw(client, build_frame_sync_prompt(in_samples, correction=error), max_tokens=16000)
    if not code.strip():
        return None, f"Claude returned no code on retry (stop_reason: {stop_reason})"

    retry_error = verify_frame_sync(code, in_samples)
    if retry_error:
        return None, f"failed self-verification twice — {retry_error}"
    return code, None
...
def assemble_full_driver(label_results, discriminators, frame_sync_code):
    ...
    if frame_sync_code:
        ...
        poll_method = '''    def poll(self) -> dict | None:
        """Either no IN-direction fields were deciphered yet, or generating/
        verifying a frame synchronizer for them failed — see the warnings
        from generation. Nothing to read/decode either way."""
        raise NotImplementedError("no usable frame synchronizer — nothing to poll")'''
